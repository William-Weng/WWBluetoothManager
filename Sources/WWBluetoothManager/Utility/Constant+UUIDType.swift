...
public extension WWBluetoothManager {
    ...
    enum UUIDType: String {
        ...
        /* 自定義 */
        case service = "0xFF10"                                         // 功能容器
        case control = "0xFF11"                                         // 傳送控制訊息
        case data = "0xFF12"                                            // 傳送實際
...
public extension WWBluetoothManager.UUIDType {
    ...
    var shortUUIDString: String { shortUUIDStringMaker() }              // 16-bit / 32-bit UUID字串（去掉 0x，並補到固定長度）
    var uuid128String: String { uuid128StringMaker() }                  // 展開成 128-bit UUID 字串
    var cbUUID: CBUUID { .init(string: rawValue) }                      // CoreBluetooth 用的 CBUUID
}

// MARK: - API
public extension WWBluetoothManager.UUIDType {
    
    /// 尋找UUIDType ("0x180F" / "180F" / "0000180F-0000-1000-8000-00805F9B34FB" => .batteryService)
    static func find(uuidString: String) -> Self? {
        let normalized = normalized16BitString(from: uuidString)
        return Self(rawValue: normalized)
    }
    
    /// 尋找UUIDType (CBUUID => .batteryService)
...
private extension WWBluetoothManager.UUIDType {
    ...
    /// [16-bit / 32-bit UUID字串（去掉 0x，並補到固定長度）](https://www.bluetooth.com/wp-content/uploads/Files/Specification/HTML/Assigned_Numbers/out/en/Assigned_Numbers.pdf)
    func shortUUIDStringMaker() -> String {

        let hex = rawValue.replacingOccurrences(of: "0x", with: "").uppercased()
        
        switch hex.count {
        case 4: return "0000\(hex)"
        case 8: return hex
        default: return hex
        }
    ...
    /// [展開成 128-bit UUID 字串](https://blog.csdn.net/hjj801006/article/details/135593595)
    func uuid128StringMaker() -> String {
        "\(shortUUIDString)\(Self.bluetoothBaseUUIDSuffix)"
    }
}
